    ...
    <div class="container px-5 py-24 mx-auto flex sm:flex-nowrap flex-wrap">
      <div class="lg:w-2/3 md:w-1/2 bg-white rounded-lg sm:mr-10 p-10 flex flex-col">
        ...
        <div class="">
            <table class="table w-full table-striped">
                <thead>
                    <tr>
                        <th class="px-4 py-2 text-gray-500 text-center">#</th>
                        <th class="px-4 py-2 text-gray-500 text-left">Name</th>
                        <th class="px-4 py-2 text-gray-500 text-center">Menus</th>
                        <th class="px-4 py-2 text-gray-500 text-center">Action</th>
                    </tr>
                </thead>
                <tbody class="text-gray-300">
                    @foreach($categories as $category)
                    <tr>
                        <th style="cursor: pointer" class="px-4 py-2">{{ $loop->index + 1 }}</th>
                        <th style="cursor: pointer" wire:click="editCategory({{ $category->id }})" class="text-left text-yellow-500 px-4 py-2">{{ $category->name }}</th>
                        <th style="cursor: pointer" class="px-4 py-2">{{ $category->menus->count() }}</th>
                        <th style="cursor: pointer" wire:click="deleteCategory({{ $category->id }})" class="text-red-500 px-4 py-2">Delete</th>
                    </tr>
      ...
      <div class="lg:w-1/3 md:w-1/2 bg-white flex flex-col md:ml-auto w-full md:py-8 mt-8 md:mt-0 p-3">
        ...
        <form wire:submit.prevent="submit">
            ...
            <button type="submit" class="text-white bg-yellow-500 border-0 py-2 px-6 focus:outline-none hover:bg-yellow-600 w-full rounded text-lg">Add More</button>
